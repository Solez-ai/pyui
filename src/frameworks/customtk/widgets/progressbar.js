...
class Progressbar extends CustomTkWidgetBase{
    ...
    generateCode(variableName, parent){
        ...
        const code = [
            `${variableName} = ctk.CTkProgressBar(master=${parent})`,
        ]

        if (mode === "determinate"){
            code.push(`${variableName}.set(${(currentValue || 0) / maxValue})`)
        }else{
            code.push(`${variableName}.start()`)
        }
