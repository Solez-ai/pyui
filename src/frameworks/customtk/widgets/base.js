...
export class CustomTkBase extends Widget {
    ...
    /**
     * Returns the customtkinter code that wires a handler to the widget.
    ...
    getEventBindCode(variableName, eventName, handlerName) {
        ...
        if (eventName === "close") {
            return `${variableName}.protocol("WM_DELETE_WINDOW", ${handlerName})`
        }

        // widgets that fire a command callback on change/click
        const commandWidgets = ["button", "check_button", "radio_button", "scale", "option_menu", "spin_box"]

        if (commandWidgets.includes(widgetType)) {
            return `${variableName}.configure(command=${handlerName})`
        }
        ...
        const bindEvent = eventBindings[eventName]

        if (!bindEvent) return ""

        return `${variableName}.bind("${bindEvent}", ${handlerName})`
    }
