...
class Stopwatch extends CustomTkWidgetBase{
    ...
    generateCode(variableName, parent){
        ...
        return [
            ...
            `        ${variableName}_label.configure(text=f"{${variableName}_counter // 100}.{(${variableName}_counter % 100) // 10}s")`,
            ...
            `    global ${variableName}_counter`,
            `    ${variableName}_counter = 0`,
            `    ${variableName}_label.configure(text="0.0s")`,
            "",
            `${variableName}_label = ctk.CTkLabel(master=${parent}, text="0.0s", font=("Arial", 20))`,
            `${variableName}_label.configure(${config})`,
            `${variableName}_start_btn = ctk.CTkButton(master=${parent}, text="Start", command=${variableName}_start, width=70)`,
            `${variableName}_stop_btn = ctk.CTkButton(master=${parent}, text="Stop", command=${variableName}_stop, width=70)`,
            `${variableName}_reset_btn = ctk.CTkButton(master=${parent}, text="Reset", command=${variableName}_reset, width=70)`,
