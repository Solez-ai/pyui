...
export function generateTkinterCodeList(widgetList = [], widgetRefs = [], parentVariable = null, mainVariable = "", usedVariableNames = new Set()) {
    ...
    for (let widget of widgetList) {
        ...
        // Add event handler functions and their bindings right after the widget code
        const eventHandlerCode = widgetRef.getEventHandlerCode(varName)
        if (eventHandlerCode.length > 0){
            code.push(...eventHandlerCode.flatMap((item, index) => index < eventHandlerCode.length - 1 ? [item, "\n"] : [item]))
            code.push("\n\n")
        }
...
/**
 * Builds the final python file string for the whole project (without downloading).
 * Returns null when the project is invalid (e.g. no Main window).
 * @param {boolean} options.silent - when true, no error messages are shown
 */
export function getTkinterCodeString(widgetList = [], widgetRefs = [], options = {}) {
    ...
    for (let widget of filteredWidgetList){
        ...
        if (mainWindowCount > 1){
            if (!silent) message.error("Multiple instances of Main window found, delete one and try again.")
            return null
        }
    ...
    if (mainWindowCount === 0){
        if (!silent) message.error("Aborting. No instances of Main window found. Add one and try again")
        return null
    }
    ...
    const code = [
        "# This code is generated by PyUI Builder",
        "\n",
        "# Developer: Samin Yeasar - https://solez.vercel.app",
        "\n\n",
...
/**
 * Generates the python code for a single widget subtree (used by the live code editor).
 * @param {object} widget - a single widget node from the widget tree {id, widgetType, children}
 * @param {object} widgetRefs - the refs map (widgetRefs.current)
 * @param {string} parentVariable - variable name of the parent widget ("" for top level)
 */
export function generateTkinterWidgetCodeString(widget, widgetRefs, parentVariable = "") {

    const generatedObject = generateTkinterCodeList([widget], widgetRefs, parentVariable, "")

    const {code: codeLines, imports} = generatedObject
...
async function generateTkinterCode(projectName, widgetList=[], widgetRefs=[], assetFiles){
    ...
    const result = getTkinterCodeString(widgetList, widgetRefs.current)

    if (!result){
        return
    }

    const {code, requirements, customPythonWidgets} = result
