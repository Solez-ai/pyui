...
class Notebook extends TkinterWidgetBase{
    ...
    generateCode(variableName, parent){
        ...
        const code = [
            `${variableName} = ttk.Notebook(master=${parent})`,
        ]

        tabs.forEach((tab, index) => {
            code.push(`${variableName}_tab_${index} = ttk.Frame(${variableName})`)
            code.push(`${variableName}.add(${variableName}_tab_${index}, text="${tab}")`)
        })
