...
class Widget extends React.Component {
        ...
        this.state = {
            ...
            attrs: {
                ...
                events: {
                    label: "Events",
                    tool: Tools.CUSTOM,
                    Component: EventHandlerEditor,
                    value: {},
                    onChange: (value) => this.setAttrValue("events", value)
                },
                // events: {
                //     event1: {
    ...
    /**
     * Returns the list of event names this widget type supports (click, change, close, etc.)
     * Override in subclasses to customize.
     * @returns {string[]}
     */
    getSupportedEvents() {
        const eventsByType = {
            main_window: ["close", "click"],
            toplevel: ["close"],
            button: ["click"],
            entry: ["change", "click"],
            Text: ["change"],
        ...
        return eventsByType[this.getWidgetType()] || []
    }

    /**
     * Returns the framework specific code line that wires a handler to the widget.
     * Override in framework bases (tkinter / customtk).
     * @param {string} variableName
     * @param {string} eventName
     * @param {string} handlerName
     * @returns {string}
     */
    getEventBindCode(variableName, eventName, handlerName) {
        return ""
    }

    /**
     * Generates the python code for the event handlers of this widget.
     * @param {string} variableName - variable name of this widget in the generated code
     * @returns {string[]} - code lines (empty array if no handlers)
     */
    getEventHandlerCode(variableName) {
        const events = this.getAttrValue("events") || {}

        const lines = []

        Object.entries(events).forEach(([eventName, code]) => {

            if (!code || !code.trim()) return // skip empty handlers

            const handlerName = `${variableName}_${eventName}_handler`

            // NOTE: event defaults to None so the handler works both with
            // .bind("...", handler) (which passes an event) and with
            // command=handler / protocol callbacks (which pass nothing)
            lines.push(`def ${handlerName}(event=None):`)
            code.split("\n").forEach(line => lines.push(`    ${line}`))
            lines.push("")
            lines.push(this.getEventBindCode(variableName, eventName, handlerName))
        })
